    ...
    public Ride createRide(CreateRideDTO dto) {
        ...
        if (activeRide.isPresent()) {
            ...
            if (!hasActivePassengers) {
                System.out.println("⚠️ Found empty stale ride " + existing.getId() + ". Auto-cancelling it to allow new ride.");
                existing.setStatus(Ride.RideStatus.CANCELLED);
                rideRepository.save(existing);
            } else {
                System.out.println("⚠️ Driver " + dto.getDriverId() + " has active ride " + existing.getId() + " with passengers. Returning existing.");
                return existing;
            }
        }
